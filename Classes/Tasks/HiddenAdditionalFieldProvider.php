...
class tx_tablecleaner_tasks_HiddenAdditionalFieldProvider implements tx_scheduler_AdditionalFieldProvider {
	...
	public function getAdditionalFields(array &$taskInfo, $task, tx_scheduler_Module $schedulerModule) {
		...
		$additionalFields[$fieldId] = array(
			'code' => $fieldHtml,
			'label' => 'LLL:EXT:tablecleaner/Resources/Private/Language/locallang.xml:tasks.hidden.tables',
			'cshKey' => 'tablecleaner',
			'cshLabel' => $fieldId,
		);
			...
			foreach ($submittedData['scheduler_tableCleanerHidden_tables'] as $table) {
				if (!in_array($table, $tables)) {
					...
					$schedulerModule->addMessage(
						$GLOBALS['LANG']->sL('LLL:EXT:tablecleaner/Resources/Private/Language/locallang.xml:tasks.hidden.invalidTables'),
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/tablecleaner/Classes/Tasks/HiddenAdditionalFieldProvider.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/tablecleaner/Classes/Tasks/HiddenAdditionalFieldProvider.php']);
}
