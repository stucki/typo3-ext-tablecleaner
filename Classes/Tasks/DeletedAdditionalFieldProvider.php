...
/**
 * Additional field provider for the Deleted scheduler task
 *
 * @package TYPO3
 ...
 */
class tx_tablecleaner_tasks_DeletedAdditionalFieldProvider implements tx_scheduler_AdditionalFieldProvider {
	...
	public function getAdditionalFields(array &$taskInfo, $task, tx_scheduler_Module $schedulerModule) {
		...
		if (empty($taskInfo['scheduler_tableCleanerDeleted_tables'])) {
			...
			if ($schedulerModule->CMD == 'add') {
				...
				if (in_array('sys_log', $this->getTables())) {
					$taskInfo['scheduler_tableCleanerDeleted_tables'][] = 'sys_log';
				}
				if (in_array('sys_history', $this->getTables())) {
					$taskInfo['scheduler_tableCleanerDeleted_tables'][] = 'sys_history';
				}
			} elseif ($schedulerModule->CMD == 'edit') {
					// In case of editing the task, set to currently selected value
				$taskInfo['scheduler_tableCleanerDeleted_tables'] = $task->getTables();
			}
		}

		$fieldName = 'tx_scheduler[scheduler_tableCleanerDeleted_tables][]';
		$fieldId = 'task_tableCleanerDeleted_tables';
		$fieldOptions = $this->getTableOptions($taskInfo['scheduler_tableCleanerDeleted_tables']);
		$fieldHtml =
		...
		$additionalFields[$fieldId] = array(
			'code' => $fieldHtml,
			'label' => 'LLL:EXT:tablecleaner/Resources/Private/Language/locallang.xml:tasks.deleted.tables',
			'cshKey' => 'tablecleaner',
			'cshLabel' => $fieldId,
		);
		...
		$fieldId = 'task_tableCleanerDeleted_dayLimit';
		$fieldCode = '<input type="text" name="tx_scheduler[scheduler_tableCleanerDeleted_dayLimit]"  id="' . $fieldId . '" value="' . htmlspecialchars($taskInfo['scheduler_tableCleanerDeleted_dayLimit']) . '"/>';
		$additionalFields[$fieldId] = array(
			'code' => $fieldCode,
	...
	public function validateAdditionalFields(array &$submittedData, tx_scheduler_Module $schedulerModule) {
		...
		if (is_array($submittedData['scheduler_tableCleanerDeleted_tables'])) {
			$tables = $this->getTables();
			foreach ($submittedData['scheduler_tableCleanerDeleted_tables'] as $table) {
				if (!in_array($table, $tables)) {
					$isValid = FALSE;
					$schedulerModule->addMessage(
						$GLOBALS['LANG']->sL('LLL:EXT:tablecleaner/Resources/Private/Language/locallang.xml:tasks.deleted.invalidTables'),
